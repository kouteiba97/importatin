...
> ## ⚠️ SUPERSEDED IN PART — read `ARCHITECTURE-LOCK.md` first
>
> This document passed a final architecture review on 14 September 2026 and **eleven decisions in
> it were revised**, mostly for over-engineering. Where this document and
> [`ARCHITECTURE-LOCK.md`](ARCHITECTURE-LOCK.md) disagree, **the lock wins**.
>
> Superseded here: the transactional outbox (pg-boss alone is transactional) · the hash-chained
> audit log (append-only grants only) · the `search.documents` index table (generated columns on
> source tables) · Algerian storage residency stated as a requirement (it is an unresolved legal
> question) · the replicated MinIO cluster (single node or managed S3) · SATIM/CIB in MVP (adapter
> + manual confirmation only) · 19 modules (**11**) · 48 tables (**34**) · ClamAV (re-encode and
> rasterise instead) · the six-state regulatory lifecycle (**three**) · Web Push in early MVP
> (Phase 8).
>
> Still authoritative here: sections 3, 5, 10–20, 26, 32, 33 and the domain reasoning throughout.
> Scope now lives in [`MVP-SCOPE.md`](MVP-SCOPE.md); sequencing in
> [`IMPLEMENTATION-ROADMAP.md`](IMPLEMENTATION-ROADMAP.md); rationale in [`adr/`](adr/).

**Phase:** pre-build · architecture only, no application code written
**Date:** 14 September 2026 · **Reviewed and locked:** 14 September 2026
