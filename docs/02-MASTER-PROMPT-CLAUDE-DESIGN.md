...
Examine the working directory and any existing code, assets, or documentation. Report what exists, what stack is in place (if any), what conventions exist, and what is greenfield. If the project is empty, say so and propose a stack appropriate to the constraints in §11 (mobile-first PWA, RTL-first, hosted in Algeria, low-bandwidth). If code exists, adapt to it rather than replacing it.
